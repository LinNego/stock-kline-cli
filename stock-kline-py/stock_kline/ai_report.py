...
from .search import search_stock, SEARCH_BACKENDS
...
def _build_user_message(
    ...
    lines.append(f"\n## 搜索结果（已为你完成联网搜索，请直接使用以下内容进行分析）")
    for sr in search_results:
        if sr.content:
            lines.append(f"\n### 搜索: {sr.query}")
            lines.append(sr.content[:2000])
        else:
            lines.append(f"\n### 搜索: {sr.query} (无结果)")
...
async def run_analyze(args: argparse.Namespace) -> None:
    ...
    if args.no_search:
        ...
        print(f"正在搜索相关信息 (后端: {SEARCH_BACKENDS.get(args.search_backend, args.search_backend)}, 模式: {args.search_mode}, 超时 {args.search_timeout}s)...")
        if args.proxy:
